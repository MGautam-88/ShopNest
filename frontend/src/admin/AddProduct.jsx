...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!image) {
      await showAlert({ title: 'Image required', text: 'Please select an image', type: 'warning' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        await showAlert({
          ...
          text: 'Product created successfully with Cloudinary Image URL!',
          type: 'success'
        });
        navigate('/shop');
      } else {
        await showAlert({
          ...
          text: responseData.message || 'Error creating product',
          type: 'error'
        });
      }
    } catch (error) {
